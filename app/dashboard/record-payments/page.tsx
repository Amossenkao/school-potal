...
function resolveFeesForStudent(
	...
	for (const { feeGroup } of feeGroups) {
		const resolved = resolveResolvedScheduledFees(
			feeGroup,
			schoolProfile,
			groupIds,
		);
		for (const rf of resolved) {
			const feeName = rf.feeDefinition?.name || rf.scheduledFee.feeId;
			const currency = rf.scheduledFee.amount.currency || 'LRD';
			const paid = payments
				.filter(
					(p) =>
...
function effectiveAmount(item: CartFeeItem): number {
	if (item.payInFull) return item.outstanding;
...
function FeeModal({
	...
	const toggleFee = (fee: ResolvedFee) => {
		const outstanding = Math.max(0, fee.amount - fee.totalPaid);
	...
	const handleSave = () => {
		...
		for (const item of items) {
			if (!item.payInFull) {
				...
				if (parsed > item.outstanding) {
					setError(
						`Amount for "${item.feeName}" exceeds the outstanding balance of ${item.currency} ${formatCurrency(item.outstanding)}.`,
					);
					return;
				}
			}
						...
						Object.entries(groupedByCategory).map(([categoryName, fees]) => (
							<div key={categoryName}>
								...
								<div className="space-y-2">
									{fees.map((fee) => {
										const remaining = Math.max(0, fee.amount - fee.totalPaid);
													...
													<div className="flex items-center gap-3 min-w-0">
														...
														<div className="min-w-0">
															<p className="text-sm font-medium text-foreground">
																{fee.feeName}
																{isCleared && (
																	<span className="ml-1.5 text-xs font-medium text-success-600">
																		Paid
																	</span>
																)}
															</p>
													...
													>
														<div className="min-w-0">
															...
															<p className="text-xs text-muted-foreground">
																...
																{item.payInFull
																	? ' · Full payment'
																	: ` · Partial (of ${item.currency} ${formatCurrency(item.outstanding)})`}
															</p>
														</div>
