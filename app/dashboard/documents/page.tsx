...
const MultiDocument = ({ students }: { students: any[] }) => (
	<Document>
...
const ManualGradeInput = ({
	...
	const handleGradeChange = (year: string, index: number, value: string) => {
		setGrades((prev) => {
			const next = { ...prev };
			(next[year as keyof typeof prev] as any)[index] = value;
			return next;
		});
	};

	const handleRankChange = (year: string, value: string) => {
		setGrades((prev) => ({ ...prev, [year]: value }));
	};

	const handleSave = () => {
		const allGrades = [...grades.grade10, ...grades.grade11, ...grades.grade12];
		if (allGrades.some((g) => !g || isNaN(Number(g)) || Number(g) < 0 || Number(g) > 100)) {
	...
	const renderGradeInputs = (year: string, yearLabel: string) => (
		<div
			key={year}
			...
			<div className="flex justify-between items-center mb-3">
				...
				<div className="flex items-center gap-2">
					...
					<input
						...
						value={grades[`rank${year.slice(-2)}` as keyof typeof grades] as string}
						onChange={(e) =>
					...
					<React.Fragment key={i}>
						<div>
							...
							<input
								...
								value={(grades[year as keyof typeof grades] as string[])[i * 2]}
						...
						<div>
							...
							<input
								type="number"
								className="w-full border-2 border-gray-300 p-2 rounded text-sm focus:border-blue-500 focus:outline-none"
								value={(grades[year as keyof typeof grades] as string[])[i * 2 + 1]}
								onChange={(e) => handleGradeChange(year, i * 2 + 1, e.target.value)}
								min="0"
								max="100"
								placeholder="0-100"
							/>
...
export default function DocumentsPage() {
	...
	const handleManualGradeUpdate = (updatedStudent: any) => {
					...
					{students.length > 0 && (
						<div className="bg-white border-2 border-gray-300 rounded-xl p-6 shadow-lg">
							...
							>
								...
								{students.length > 1 ? 's' : ''} &times; 2 documents ={' '}
