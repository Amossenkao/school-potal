...
const SubmitGrade: React.FC<GradeSubmitProps> = ({
		...
		try {
			...
			const initialStudentsForGrading = studentsList.map((student: any) => {
				...
				periods.forEach(({ id, value }) => {
					// Fix: Only use id as key if it exists in periodIdToApiKey
					const apiPeriodKey =
						id in periodIdToApiKey
							? periodIdToApiKey[id as keyof typeof periodIdToApiKey]
							: id;
					const existingGrade = studentExistingPeriods[apiPeriodKey];

					// Updated logic: Only show grades with 'Approved' or 'Pending' status
					// If grade is 'Rejected' or doesn't exist, show input field
					if (
						existingGrade &&
						...
						// Show input field for new grades, rejected grades, or missing grades
						grades[id] = {
							grade: '',
	...
	// Only trigger loadStudentsForGrading when all selections are made and stable
	useEffect(() => {
		if (
			selectedSession &&
			selectedClassLevel &&
			...
			selectedSubject &&
			selectedAcademicYear &&
			!loading.studentsForGrading // Prevent re-triggering while loading
		) {
			loadStudentsForGrading();
		}
		// Only depend on primitive values, not objects/arrays
		...
		<div className="space-y-6">
			<style jsx>{`
				...
				.custom-scrollbar::-webkit-scrollbar-thumb {
					...
					// app/dashboard/teacher/grading/SubmitGrade				.custom-scrollbar::-webkit-scrollbar-thumb:hover {
					background: linear-gradient(
						135deg,
						#7c3aed 0%,
						#6d28d9 50%,
											...
											return (
												...
												>
													<div className="relative space-y-1 flex flex-col items-center">
														{isExisting ? (
															<div className="flex flex-col items-center space-y-1 cursor-not-allowed">
																<div
