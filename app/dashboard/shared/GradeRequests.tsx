...
const normalizeBulkRequests = (input: unknown): BulkGradeRequest[] => {
	...
	input.forEach((item, itemIndex) => {
		...
		const batchId = toStringSafe(
			item.batchId ?? item.submissionId,
			fallbackBatchId,
		);
...
const GradeRequests: React.FC<GradeRequestsProps> = ({
	...
	const teacherAcademicYears = useMemo(
		() => (effectiveUser ? getTeacherAcademicYears(effectiveUser) : []),
		[effectiveUser],
	);
	const allowedAcademicYears = useMemo(
		() =>
			sortAcademicYearsDesc(
				getAllowedGradeChangeRequestAcademicYears(currentSchool) || [],
			),
		[currentSchool],
	);
	...
	}, [availableAcademicYears, currentAcademicYear, isTeacher]);
	...
	}, [isTeacher, selectedAcademicYear, allowedAcademicYears]);
	...
	}, [availableAcademicYears, currentAcademicYear, selectedAcademicYear, isTeacher]);
	...
	const fetchRequests = useCallback(
		...
		[selectedAcademicYear, setGradeRequestsForYear],
	);

	useEffect(() => {
		void fetchRequests();
	}, [selectedAcademicYear, fetchRequests]);
	...
	const applyRequestStatusLocally = (payload: {
		...
		const nextRequests = bulkRequests.map((batch) => {
			...
			const nextBatchRequests = batch.requests.map((request) => {
				if (
					!targetIds.has(request.requestId) ||
	...
	const handleOpenEditModal = (request: GradeChangeRequest) => {
		setEditModal({
			...
			originalGrade: request.originalGrade ?? 0,
	...
	const renderDetailsModal = () => {
		...
		return (
			...
			>
				<div
					className="bg-card rounded-lg shadow-xl w-full max-w-5xl max-h-[calc(100dvh-3rem)] flex flex-col border"
	...
	const renderRejectModal = (isBulk: boolean) => (
		...
		>
			<div
				className="bg-card rounded-lg shadow-xl w-full max-w-md max-h-[calc(100dvh-3rem)] overflow-y-auto overscroll-contain border"
				onClick={(e) => e.stopPropagation()}
			>
	...
	return (
		<div className="space-y-6 bg-background text-foreground p-4 md:p-6">
			{/* Teacher edit modal */}
			{editModal.isOpen && renderEditModal()}
			...
			<div className="bg-card border rounded-lg">
				...
				<div className="p-6">
					<div className="flex flex-col gap-4 sm:flex-row sm:justify-between sm:items-start mb-4">
						<div>
							...
							<p className="text-muted-foreground mt-1">
								{isTeacher
									? 'View, edit, or withdraw your grade change requests.'
									: 'Review, approve, or reject teacher requests to change student grades.'}
								{selectedAcademicYear
					...
					{isTeacher && !isSelectedAcademicYearAllowed && (
						<div className="mb-4 rounded-lg border border-amber-200 bg-amber-50 p-4 text-amber-800 text-sm">
							Grade change requests are not allowed for academic year{' '}
				...
				{!isTeacher && selectedBulkRequests.size > 0 && (
				...
				<div className="border-t">
					{loading ? (
						...
						<div className="p-6 text-center text-muted-foreground">
							{isTeacher
								? 'You have not submitted any grade change requests.'
									...
									<thead className="bg-muted/50">
										<tr>
											{!isTeacher && (
												<th className="p-3 text-left">
													<input
														type="checkbox"
														onChange={(e) =>
															toggleAllRequests(e.target.checked)
														}
														checked={
											...
											<th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">
												{isTeacher ? 'Subject / Class' : 'Teacher / Subject'}
											</th>
											{!isTeacher && (
												<th className="px-6 py-3 text-left text-xs font-medium text-muted-foreground uppercase">
													Class
											...
											<tr key={batch.batchId}>
												{!isTeacher && (
													<td className="p-3">
														<input
															type="checkbox"
															checked={selectedBulkRequests.has(
																batch.batchId,
															)}
															onChange={() =>
													...
													{isTeacher ? (
														<div>
															...
															<div className="text-sm text-muted-foreground">
																{classMap.get(batch.classId) || batch.classId}
															</div>
														</div>
													) : (
														<>
