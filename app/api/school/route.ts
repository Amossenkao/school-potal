...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const newSchool = await SchoolProfile.create({
			system: {
				isActive: true,
				host: body.system?.host || host,
				dbName: body.system?.dbName || dbName,
			},
			identity: {
				name,
				...
				currentAcademicYear: body.identity?.currentAcademicYear || currentYear,
			},
			branding: {
				logoUrl: body.branding?.logoUrl || '',
				logoUrl2: body.branding?.logoUrl2 || '',
				themeName: body.branding?.themeName || DEFAULT_TENANT_THEME_NAME,
			...
			userConfig: {
				...
				administrativePositions: body.userConfig?.administrativePositions || [],
			...
			featureConfig: {
				enabledFeatures: body.featureConfig?.enabledFeatures || ['dashboard', 'user_management', 'profile_management', 'homepage'],
