...
export async function GET(request: NextRequest) {
	try {
		...
		if (currentUser.role === 'teacher') {
			 ...
			 */
			// `getSchoolProfile` returns the nested document — the class tree lives
			// under `academicConfig`, never at the root.
			const classLevels = schoolProfile?.academicConfig?.classLevels;
			const taughtLevels = new Set(
				assignedClassIds
					.map((id) => getClassMetaById(classLevels, id))
			...
			if (taughtLevels.size === 0) {
				// The class tree could not be read, so no level can be verified.
				// Narrow to their own classes rather than deny or over-share.
				classScope = classId ? [classId] : assignedClassIds;
			} else if (classId) {
				const meta = getClassMetaById(classLevels, classId);
