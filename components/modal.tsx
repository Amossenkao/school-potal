...
const MultiStepUserModal = ({ showModal, onClose, onUserCreated }) => {
  ...
  const subjectsByLevel = {
    "Self Contained": [
      'Reading', 'Writing', 'Basic Math', 'Science Discovery', 'Social Awareness',
      'Art & Crafts', 'Music & Movement', 'Physical Play', 'Life Skills', 'Story Time'
    ],
    "Elementary": [
      'Mathematics', 'English Language Arts', 'Science', 'Social Studies', 
      'Art', 'Music', 'Physical Education', 'Health Education', 'Computer Basics',
      'Library Skills', 'Character Education'
    ],
    "Junior High": [
      'Mathematics', 'English', 'Science', 'Social Studies', 'Physical Education',
      'Art', 'Music', 'Technology', 'Health & Wellness', 'Study Skills',
      'Career Exploration', 'Foreign Language'
    ],
    "Senior High": [
      'Advanced Mathematics', 'English Literature', 'Biology', 'Chemistry', 'Physics',
      'World History', 'Government', 'Economics', 'Physical Education', 'Art',
      'Music', 'Computer Science', 'Foreign Language', 'Psychology', 'Philosophy',
      'Advanced Placement Courses', 'Vocational Training'
    ]
  };
  ...
  const adminPositions = [
    ...
    'Guidance Counselor', 'Librarian', 'Secretary', 'Administrative Assistant'
  ];
  ...
  const adminPermissions = [
    ...
    'disciplinary_actions', 'parent_communication'
  ];
  ...
  const handleSelfContainedSelection = (classId, checked) => {
    if (checked) {
      ...
      const selfContainedSubjects = subjectsByLevel["Self Contained"].map(subject => ({
        subject,
    ...
    switch (userType) {
      case 'student':
        userData = {
          ...
          currentClass: classIds.find(c => c.id === formData.classId)?.name,
          grade: classIds.find(c => c.id === formData.classId)?.name,
          status: 'enrolled',
          requiresOtp: false,
  ...
  const resetForm = () => {
    ...
    setFormData({
      ...
      subjects: [], isSponsor: false, sponsorClass: null, position: '', permissions: []
    });
    setErrors({});
  };
  ...
  const isClassSelfContained = (classId) => {
    const classInfo = classIds.find(c => c.id === classId);
    return classInfo?.level === "Self Contained";
  };

  if (!showModal) return null;

  return (
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 w-full h-screen">
      <div className="bg-background rounded-lg border shadow-xl w-full max-w-5xl max-h-[90vh] overflow-hidden">
        {/* Header */}
              ...
              {userType === 'teacher' && (
                <>
                  ...
                  <div className="border border-border rounded-lg p-4 bg-muted/30">
                    <h4 className="font-medium text-foreground mb-3">Self-Contained Class Teacher</h4>
                    <p className="text-sm text-muted-foreground mb-4">
                      Self-contained teachers teach all subjects to one class and are automatically sponsors of that class.
                    </p>
                    
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                      {getSelfContainedClasses().map((cls) => (
                        <label key={cls.id} className="flex items-center p-2 border border-border rounded hover:bg-muted/50 cursor-pointer">
                          <input
                            type="radio"
                    ...
                    {formData.sponsorClass && (
                      <button
                        type="button"
                        onClick={() => handleSelfContainedSelection(null, false)}
                  ...
                  {!formData.sponsorClass && (
                    <div>
                      ...
                      <div className="space-y-4 max-h-96 overflow-y-auto border border-border rounded-lg p-4">
                        {classLevels.filter(level => level !== "Self Contained").map((level) => (
                          <div key={level} className="border-b border-border pb-4 last:border-b-0">
                            <h5 className="font-medium text-foreground mb-3">{level}</h5>
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                              {subjectsByLevel[level]?.map((subject) => (
                                <label key={`${subject}-${level}`} className="flex items-center text-sm">
                                  <input
                                    type="checkbox"
                  ...
                  {!formData.sponsorClass && (
                    <div>
                      ...
                      >
                        ...
                        {classIds.filter(cls => cls.level !== "Self Contained").map((cls) => (
                          <option key={cls.id} value={cls.id}>
                            {cls.name} ({cls.level})
                          </option>
                        ))}
                  ...
                  {formData.subjects.length > 0 && (
                    <div className="bg-muted/30 border border-border rounded-lg p-4">
                      <h5 className="font-medium text-foreground mb-2">Teaching Summary</h5>
                      <div className="space-y-2">
                        {formData.sponsorClass && (
                          <p className="text-sm text-foreground">
                            <span className="font-medium">Self-Contained Class:</span> {classIds.find(c => c.id === formData.sponsorClass)?.name}
                          </p>
                        )}
                        <p className="text-sm text-foreground">
                          <span className="font-medium">Total Subjects:</span> {formData.subjects.length}
                        </p>
                        {formData.isSponsor && (
                          <p className="text-sm text-foreground">
                            <span className="font-medium">Class Sponsor:</span> Yes
                          </p>
                        )}
                      </div>
                    </div>
              ...
              <div className="border-t pt-6">
                ...
                <div className="bg-muted/30 rounded-lg p-4 space-y-3">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-sm font-medium text-foreground">{formData.gender}</span>
                  </div>
                  <div className="flex justify-between">
                  ...
                  {userType === 'teacher' && (
                    <>
                      {formData.sponsorClass && (
                        <div className="flex justify-between">
                          <span className="text-sm text-muted-foreground">Teaching Type:</span>
                          <span className="text-sm font-medium text-foreground">Self-Contained</span>
                        </div>
                      )}
                      <div className="flex justify-between">
                      ...
                      {formData.isSponsor && (
                        <div className="flex justify-between">
                          <span className="text-sm text-muted-foreground">Class Sponsor:</span>
                          <span className="text-sm font-medium text-foreground">
                            {classIds.find(c => c.id === formData.sponsorClass)?.name || 'Yes'}
                          </span>
                        </div>
                      )}
                    </>
