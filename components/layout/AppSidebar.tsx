...
const AppSidebar: React.FC = () => {
	...
	useEffect(() => {
		const fetchPendingCounts = async () => {
			if (user) {
				try {
					...
					// --- Fetch Pending Grade Change Requests (API handles role) ---
					const requestsRes = await fetch(
						`/api/grades/requests?academicYear=${currentYear}`
					);
					if (requestsRes.ok) {
					...
					// --- Fetch Pending Grade Submissions (Role-dependent) ---
					if (user.role === 'system_admin') {
						//
						// Admin: Fetch ONLY pending grades and count unique submission IDs
						const submissionsRes = await fetch(
							`/api/grades?academicYear=${getCurrentAcademicYear()}&status=Pending`
						);
						if (submissionsRes.ok) {
							const submissionsData = await submissionsRes.json();
							// This array now only contains 'Pending' grades
							const rawGrades = submissionsData.data?.report?.grades;
							if (rawGrades && Array.isArray(rawGrades)) {
								// Count unique submissionIds from the list of pending grades
								const pendingSubmissionIds = new Set(
									rawGrades.map((grade) => grade.submissionId)
								);
								const pendingCount = pendingSubmissionIds.size;
		...
		fetchPendingCounts();
	}, [user]); // Re-fetch counts when user loads
		...
		if (currentSchool && role) {
			try {
				...
				const processedNavItems = completeNavItems.map((item) => {
					const newItem = {
						...
						subItems: item.subItems
							? item.subItems.map((sub) => {
									let badgeCount;
									if (
										sub.name === 'Grade Submissions' ||
										sub.name === 'Manage Submissions' // Handle admin naming
									) {
										badgeCount = pendingSubmissionsCount;
									} else if (
										sub.name === 'Grade Requests' ||
										sub.name === 'Manage Requests' // Handle admin naming
									) {
					...
					// Sum up counts for the main category badge
					if (item.name === 'Academics' || item.name === 'Grading') {
						...
						{subItems ? (
							...
							>
								...
								{(isExpanded || isHovered || isMobileOpen) &&
									item.badgeCount &&
									item.badgeCount > 0 && (
										<span className="ml-auto bg-red-500 text-white text-xs font-bold rounded-full h-5 min-w-[1.25rem] px-1 flex items-center justify-center">
											{item.badgeCount}
										</span>
									)}
								{(isExpanded || isHovered || isMobileOpen) && (
